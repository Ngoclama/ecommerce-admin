...
import { SizeColumn, useSizeColumns } from "./columns";
...
export const SizeClient: React.FC<SizeClientProps> = ({ data }) => {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      setIsLoading(true);
      const sizeIds = selectedRows.map((row) => row.id);
      const response = await fetch(`/api/${params.storeId}/sizes`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ ids: sizeIds }),
      });

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.message || "Failed to delete sizes");
      }

      toast.success(
        `${selectedRows.length} ${
          t("actions.deleteSelectedSuccess") || "sizes deleted successfully!"
        }`
      );
      setSelectedRows([]);
      router.refresh();
    ...
    <>
      ...
      <AlertModal
        ...
        title={
          ...
          `Delete ${selectedRows.length} selected sizes?`
        }
        description={
      ...
      <div className="flex items-center justify-between">
        <Heading
          title={`${t("nav.sizes")} (${data.length})`}
          description={t("nav.sizes")}
        />
